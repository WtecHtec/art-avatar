...
export default function Content({ selectedFeatures }: ContentProps) {
  ...
  // 生成头像的核心逻辑
  const generateAvatar = (useSelected: boolean = true, prevFeatures: any = null) => {
    // 完全随机生成
    if (!useSelected) {
    ...
    if (!prevFeatures && Object.keys(selectedFeatures).length === 0) {
     ...
     return {
      ...baseFeatures,
      ...Object.entries(selectedFeatures).reduce((acc, [type, shape]) => ({
        ...acc,
        [type]: { 
  ...
  // 生成按钮文本
  const generateButtonText = Object.keys(selectedFeatures).length > 0 
    ? 'Randomize' 
    : 'Generate';
    ...
    <div className="max-w-md mx-auto pt-20">
      ...
      <div className="text-center text-sm text-gray-500">
        {Object.keys(selectedFeatures).length > 0 
          ? 'Click randomize to ignore selections and generate random avatar.'
